...
namespace SQLEADOR.Conexiones
{
    class Mysql
    {
        ...
        MySqlConnection cnn;
        public Mysql(Conexion conexion)
        {
            cnn = new MySqlConnection(String.Format(connectionString, conexion.MysqlInfo()));
            cnn.Open();
        }
        ...
        public Boolean subirAlumno(DataRow Alumno)
        {
            try
            {
        ...
        public Boolean subirCalificaciones(DataTable Calificaciones)
        {
            try
            {
        ...
        public Boolean subirKardex(DataTable Kardex)
        {
            try
            {
                ...
                StringBuilder sql = new StringBuilder(
                    "insert into " +
                    "kardexes(matricula,nombre_mat,periodo,calificacion,cuatrimestre) " +
                    "values ");
                MySqlCommand cmd = new MySqlCommand();
                cmd.Connection = cnn;
        ...
        public Boolean subirHorario(DataTable Horario)
        {
            try
            {
        ...
        public static bool testConnection(Conexion mysql)
        {
            string connectionString = "server={0};port={1};database={2};uid={3};pwd={4};SslMode=none";
            try
            {
                using (var conn = new MySqlConnection(String.Format(connectionString, mysql.MysqlInfo())))
                {
                    conn.Open();
                    return true;
                }
